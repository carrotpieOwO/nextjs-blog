...
export default async function handler(
  ...
  if(Array.isArray(data.files.image)){
    // const newFilenames = data.files.image.map((file: PersistentFile) => file.newFilename);
    // return res.status(201).json({ message: newFilenames });
  } else {
    return res.status(201).json({ message: data.files.image.newFilename });
  }
}
